var linesPerson = function() {
	...
	this.cylinder.position.set(0, -1, -0.4);
	this.hitBallMachineHandRight.add( this.cylinder );

	this.plane = new THREE.Mesh( new THREE.PlaneGeometry( 0.4, 0.7, 32 ), new THREE.MeshBasicMaterial( {color: 0xff0000, side: 	THREE.DoubleSide} ) );
	this.plane.rotation.z = -1.5 + Math.PI/2;
	this.plane.rotation.y = Math.PI/2;
	this.plane.position.set(0, -1.32, -0.82);
...
linesPerson.prototype.update = function() {
	if(this.isOn) {
  	if(this.inOrOut === 1) {
    	if(this.hitBallMachineHandRight.rotation.x < 1.5) {
